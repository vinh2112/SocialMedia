...
export const createPost = async (req, res) => {
  try {
    ...
    const { followers } = await UserModel.findById(req.userId).select("followers").lean();

    NotificationService.createNewNotification(req.userId, followers, "new-post", newPost._id);
...
async function handleUpdateScoreByAction(categories, ipAddress, behaviorType, scoreInc = 1) {
  ...
  for await (const category of categories) {
    await KeywordService.updateScoreOfKeyword(category, ipAddress, behaviorType, scoreInc, tempCateLength-- / categories.length);
  }
}
