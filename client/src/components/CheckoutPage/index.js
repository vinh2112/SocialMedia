...
const PaySection = ({ post }) => {
  ...
  const handleMomoCheckout = async () => {
    setLoading(true);
    await MomoAPI.getPayUrl({ url: window.location.href, price: post.price }).then((res) => {
          ...
          <BottomSection>
            <div className="payment__options">
              <PaymentOptionItem name="payment__option" value={1} onChange={handlePaymentType} checked={true}>
                ...
                <div className={`payment__option-content_body ${currentUser && currentUser.wallet > 0 ? "green" : "red"}`}>
              ...
              <>
                ...
                <div className="payment__options-list_Ewallet">
                  <EWalletItem
                    ...
                    checked={true}
                  />
                  <EWalletItem backgroundImage={Momo} name="ewallet-option" value="momo" onChange={handleChangeEWallet} />
                  <EWalletItem backgroundImage={ZaloPay} name="ewallet-option" value="zalopay" onChange={handleChangeEWallet} />
                  <EWalletItem backgroundImage={VNPAY} name="ewallet-option" value="vnpay" onChange={handleChangeEWallet} />
                </div>
              </>
            )}
          </BottomSection>
...
const EWalletItem = (props) => {
  return (
    <EWalletContainer style={{ backgroundImage: `url(${props.backgroundImage})` }} onChange={() => props.onChange(props.value)}>
      <input type="radio" hidden name={props.name} defaultChecked={props.checked} />
